  ...
  <main class="work-showcase">
    <div class="work-item" v-for="item in works">
      ...
      <div class="description">
        ...
        <h2 class="subheadline">{{item.subtitle}}</h2>
        <ul class="stats text">
          <li v-for="stat in item.stats"><span>{{stat.name}}</span>{{stat.value}}</li>
        </ul>
        <a :href="item.url" class="launch" :class="inactive(item.url)">
          {{launchText(item.url)}}
        </a>
      </div>
    </div>
